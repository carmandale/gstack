...
function generatedForClaudeHost(tmpl: string): boolean {
  const skillDir = path.basename(path.dirname(path.join(ROOT, tmpl)));
  const generation = getHostConfig('claude').generation;
  if (generation.includeSkills?.length && !generation.includeSkills.includes(skillDir)) return false;
  if (generation.skipSkills?.includes(skillDir)) return false;
  return true;
}
...
for (const { tmpl, output } of TEMPLATES) {
  ...
  if (!generatedForClaudeHost(tmpl)) {
    console.log(`  -  ${tmpl.padEnd(30)} — skipped by Claude Code generator`);
    continue;
  }
